      ...
      .flatMap { json =>
        IO.fromEither(
          decode[List[BanachThreshold]](json)
            .leftMap(e => RuntimeException(s"JSON decode failed: ${e.getMessage}"))
        )
      }

  /** ファイルパスから Dhall 式を読み込んで閾値を返す。 */
  def loadThresholdsFromFile(path: os.Path): IO[List[BanachThreshold]] =
    IO.blocking(os.read(path)).flatMap(loadThresholds)
