...
//    §3  PerlBridge.generatePerl    — Perl コード生成
...
class PerlBridgeSpec extends AnyFunSpec with Matchers:
    ...
    it("Scalar プログラム → ScalarTyped") {
      ...
      ProgramLifter.liftTyped(prog) match
        case Right(ProgramLifter.ScalarTyped(expr)) =>
          // expr の型は TypedExpr[Ty.Scalar.type] — コンパイル時に保証
  ...
  // ── §3  PerlBridge.generatePerl — コード生成 ─────────────────────────────

  describe("PerlBridge.generatePerl") {

    it("PushScalar → push @stack, N;") {
      val code = PerlBridge.generatePerl(
        Vector(Instr.PushScalar(42)),
        """print $stack[-1], "\n";"""
      )
      code should include ("push @stack, 42;")
    }

    it("AddScalar → pop × 2 → push sum") {
      val code = PerlBridge.generatePerl(
        Vector(Instr.PushScalar(3), Instr.PushScalar(4), Instr.AddScalar),
        """print $stack[-1], "\n";"""
      )
      code should include ("push @stack, 3;")
      code should include ("push @stack, 4;")
      code should include ("$b + $a")
    }

    it("PushVec3 → push @stack, [x, y, z];") {
      val code = PerlBridge.generatePerl(
        Vector(Instr.PushVec3(1, 2, 3), Instr.PushVec3(4, 5, 6), Instr.DotVec3),
        ProgramLifter.perlPrintFor(
          ProgramLifter.liftTyped(Vector(Instr.PushVec3(1,2,3), Instr.PushVec3(4,5,6), Instr.DotVec3))
            .getOrElse(ProgramLifter.ScalarTyped(TypedExpr.TConstScalar(0)))
        )
      )
      code should include ("[1, 2, 3]")
      code should include ("[4, 5, 6]")
      // DotVec3 → scalar result
      code should include ("$b->[0]*$a->[0]")
    }

    it("生成コードが use strict を含む") {
      val code = PerlBridge.generatePerl(Vector(Instr.PushScalar(1)), "print 1;")
      code should include ("use strict")
      code should include ("use warnings")
      code should include ("my @stack")
    }
  }
  ...
  private def perlEnabled: Boolean =
    sys.env.get(PerlEnv).exists(v => v == "1" || v.equalsIgnoreCase("true"))
  ...
  describe("PerlBridge.crossCheck (integration, requires Strawberry Perl)") {
    ...
    it("BatchJob.dhall の frobenius-compile ステップ: 12 * 1 = 12") {
      ensurePerl()
      // BatchJob.dhall: PushScalar(12), PushScalar(1), MulScalar
    ...
    it("BatchJob.dhall の padic-lower ステップ: (2,4,0)·(1,0,8) = 2") {
      ensurePerl()
      // BatchJob.dhall: PushVec3(2,4,0), PushVec3(1,0,8), DotVec3
      val prog = Vector(
